...
const CATEGORIES = {
  stats: {
    ...
    description: 'Performance data, player cards, leaderboards',
    color:       theme.STATS,
    commands: [
      { name: '!stats [@user]',         desc: 'Full player card with rank tier badge and live delta indicators' },
      { name: '!leaderboard',           desc: 'Hall of fame — full group rankings, updated live from WebSocket' },
      { name: '!leaderboard --partial', desc: 'Secondary partial-board view' },
    ],
  },
  system: {
  ...
  moderation: {
    ...
    color:       theme.DANGER,
    commands: [
      { name: '!ban @user [reason]',   desc: 'Ban a member — shows confirmation prompt before executing' },
      { name: '!unban <userId> [reason]', desc: 'Unban a member — shows confirmation prompt before executing' },
